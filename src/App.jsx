...
export default function App() {
  const [activeTab, setActiveTab] = useState('desktop'); // 'desktop' or '3d-game'
  ...
  return (
    <>
      ...
      {/* OS Top Bar with Tabs */}
      <header className="os-topbar">
        <div className="os-topbar-left">
          <span>⚡ URIA OS v2.0</span>
          <div className="os-tabs">
            <button 
              className={`os-tab ${activeTab === 'desktop' ? 'active' : ''}`}
              onClick={() => setActiveTab('desktop')}
            >
              💻 Workspace
            </button>
            <button 
              className={`os-tab ${activeTab === '3d-game' ? 'active' : ''}`}
              onClick={() => setActiveTab('3d-game')}
            >
              🏀 3D Court
            </button>
          </div>
        ...
        <div className="os-topbar-right">
          <span className="os-score">SCORE: {score}</span>
          <span>{clock}</span>
        </div>
      </header>
      ...
      {activeTab === 'desktop' ? (
        <main className="os-desktop">
          ...
          <section className="hero-section">
            ...
            <div className="hero-cta-row">
              ...
              <button className="btn-neon secondary" onClick={() => setActiveTab('3d-game')}>
                🏀 Enter 3D Court
              </button>
            </div>
          </section>
        ...
        /* ── 3D Basketball Game Tab ── */
        <Basketball3D setGlobalScore={setScore} />
      )}
    </>
  );
}
